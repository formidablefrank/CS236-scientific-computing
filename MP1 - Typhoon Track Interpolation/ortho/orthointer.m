...
%hermite polynomial interpolation function for lambda
i_lambda = hermite(t, lambda, query);
...
xlabel('t');
ylabel('\lambda');
title('Hermite Polynomial Interpolation in \lambda');
%daspect([1 1 1]);

%hermite polynomial interpolation function for phi
i_phi = hermite(t, phi, query);
...
xlabel('t');
ylabel('\phi');
title('Hermite Polynomial Interpolation in \phi');
%daspect([1 1 1]);

%now, plot lambda vs. phi for position
figure;
plot(i_lambda([18:end-18]),i_phi([18:end-18]),'-');
xlabel('\lambda');
ylabel('\phi');
title('Hermite Polynomial Interpolation in position');
%daspect([1 1 1]);

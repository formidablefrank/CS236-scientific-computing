...
%lagrange interpolation function for lambda
i_lambda = lagrangefunc(t, lambda, query);
...
xlabel('t');
ylabel('\lambda');
title('Lagrange Interpolation in \lambda');
%daspect([1 1 1]);

%lagrange interpolation function for phi
i_phi = lagrangefunc(t, phi, query);
...
xlabel('t');
ylabel('\phi');
title('Lagrange Interpolation in \phi');
%daspect([1 1 1]);
...
xlabel('\lambda');
ylabel('\phi');
title('Lagrange Interpolation in position');
%daspect([1 1 1]);

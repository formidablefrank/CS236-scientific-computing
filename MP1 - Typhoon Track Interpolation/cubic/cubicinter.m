...
pp_lambda = csape(t,lambda,'periodic');
...
xlabel('t');
ylabel('\lambda');
title('Cubic Spline Interpolation in \lambda');
daspect([1 1 1]);

%cubic spline interpolation function for phi
pp_phi = csape(t,phi,'periodic');
...
xlabel('t');
ylabel('\phi');
title('Cubic Spline Interpolation in \phi');
daspect([1 1 1]);
...
xlabel('\lambda');
ylabel('\phi');
title('Cubic Spline Interpolation in position');
daspect([1 1 1]);
